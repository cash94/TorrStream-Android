...
import my.torrstream.app.MainActivity.Companion.VIDEO_COMPLETED_DURATION_MAX_PERCENTAGE
...
class PlayerStateManager(context: Context) {
    ...
    /**
     * Lists persisted playback states worth surfacing in an Android TV "Continue Watching" row:
     * has a LampaCard attached, has real progress, and isn't finished yet.
     *
     * @param maxCount Maximum number of states to return.
     * @return States sorted by most recently played first.
     */
    fun getContinueWatchingCandidates(maxCount: Int = 10): List<PlaybackState> {
        return prefs.all.keys
            .filter { it.startsWith("state_") }
            .mapNotNull { loadPersistedState(it.removePrefix("state_")) }
            .filter { state ->
                !state.isEnded &&
                        state.extras.containsKey(LAMPA_CARD_KEY) &&
                        ((state.currentItem?.timeline?.time ?: 0.0) > 0.0 || state.currentPosition > 0)
            }
